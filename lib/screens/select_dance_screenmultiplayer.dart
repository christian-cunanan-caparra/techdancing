...
class SelectDanceScreenMultiplayer extends StatefulWidget {
  final Map user;
  final String roomCode;

  const SelectDanceScreenMultiplayer({
    super.key,
    required this.user,
  ...
  @override
  State<SelectDanceScreenMultiplayer> createState() => _SelectDanceScreenMultiplayerState();
}

class _SelectDanceScreenMultiplayerState extends State<SelectDanceScreenMultiplayer>
  ...
  void _selectRandomDance() async {
    ...
    try {
      // Use the multiplayer-specific API endpoint
      final result = await ApiService.selectRandomDanceMulti(widget.roomCode);

      if (result['status'] == 'success') {
        final danceId = result['dance_id'];
        await _handleDanceSelection(danceId);
      } else {
        // Fallback to client-side random selection if server fails
        final randomIndex = _random.nextInt(dances.length);
        final selectedDance = dances[randomIndex];

        // Try to notify the server about our selection
        try {
          await ApiService.selectDanceMulti(widget.roomCode, selectedDance['id']);
        } catch (e) {
          // If server call fails, just proceed with local selection
          debugPrint("Failed to notify server: $e");
        }

        await _handleDanceSelection(selectedDance['id']);
      }
    } catch (e) {
      debugPrint("Dance selection error: $e");

      // Final fallback - deterministic based on room code
